...
const createComment = async (req, res) => {
  ...
  try {
    ...
    if (!sent) res.status(200).json(comment);
  } catch (error) {
...
const deleteComment = async (req, res) => {
  try {
    const { id } = req.params;
    let sent = false;
    let response = await Comment.findByIdAndUpdate(
      id,
      { isDeleted: true },
      { new: true },
      async function (error, result) {
        if (error && !sent)
          res
            .status(400)
            .send("Deletion of comment " + id + " failed with error " + error);
    ...
    if (!sent) res.status(200).send("Comment deleted successfully.");
  } catch (e) {
