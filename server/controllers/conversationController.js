...
const newConversation = async (req, res) => {
  ...
  try {
    ...
    let response = await User.findOne(
      ...
      async function (errS, resultS) {
        if (errS || !resultS) {
          ...
          await User.findOne(
            { email: receiverEmail },
            function (errR, resultR) {
              if (errR || !resultR) {
                if (!sent) {
                  res
                    .status(400)
                    .send("Receiver email not found or error occurred.");
                  sent = true;
                }
              }
            }
    ...
    response = await Conversation.findOne(
      {
        $or: [
          { members: [senderEmail, receiverEmail] },
          { members: [receiverEmail, senderEmail] },
        ],
      },
      function (error, result) {
        if (result) {
          if (!sent) {
            res
              .status(400)
              .send("These members already have an existing conversation.");
            sent = true;
          }
        }
      }
...
module.exports = {
  newConversation,
  getAllConversation,
  getSpecificConversationBuUsers,
};
