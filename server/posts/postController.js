  ...
  try {
    await Post.find({ isDeleted: false }, function (err, docs) {
      if (!req || !res) {
        ...
        if (err) return null;
        return docs;
      } else if (err) res.status(400).json({ message: err });
      else if (docs) {
...
const getTagsFrequencies = async (req, res) => {
  try {
    const result = await getPopularPostThemes(
      req.body.tag1,
      req.body.tag2,
      req.body.tag3
    );
      ...
      function (error, docs) {
        if (error || !docs)
          if (!sent) {
            ...
            if (!sent) res.status(200).json(docs);
          }
      }
    ).clone();
  } catch (e) {
...
const createPost = async (req, res) => {
  ...
  try {
    ...
    if (!sent) res.status(200).json(post);
  } catch (error) {
    if (!sent) res.status(404).json({ message: error });
  }
};
...
const deleteCommentFromPost = async (req) => {
  ...
  try {
    let response = await Post.findById(
      req.postID,
...
const updatePost = async (req, res) => {
  try {
      ...
      function (error, docs) {
        if (error || !docs) {
          if (!sent) {
            res
              .status(400)
              .send("No user email with the email provided - " + userEmail);
            sent = true;
          }
        }
      }
    ...
    if (content) resDoc.content = content;
    if (image) resDoc.image = image;
    if (allCommentIDs) resDoc.allCommentIDs = allCommentIDs;
    await Post.findByIdAndUpdate(
      id,
      resDoc,
      { new: true },
  ...
  try {
    ...
    await Post.findByIdAndUpdate(
      ...
      async function (error, result) {
        ...
        if (!sent) res.status(200).send("Post deleted successfully.");
      }
    ).clone();
  } catch (e) {
